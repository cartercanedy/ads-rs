...
public class RouterService : BackgroundService {
  ...
  public RouterService(IConfiguration configuration, ILoggerFactory loggerFactory) {
    ...
    _configuration.Dump(_logger, LogLevel.Information);
  ...
  protected override async Task ExecuteAsync(CancellationToken cancel) {
    ...
    var adsRouterService = new AdsRouterServer(router, _loggerFactory);

    var adsEmu = new AdsDeviceEmulator(351, "AdsDeviceEmu", _loggerFactory);

    // And a simple TwinCAT System Service (AmsPort 10000) for supporting browsing routes (including BroadcastSearch)
    var systemService = new SystemServiceServer(router, _loggerFactory);

    var systemServiceTask = systemService.ConnectServerAndWaitAsync(cancel);
    var routerServerTask = adsRouterService.ConnectServerAndWaitAsync(cancel);
    var adsEmuTask = adsEmu.ConnectServerAndWaitAsync(cancel);

    // Wait until Router, AdsRouter Server and SystemService Server have finished.
    await Task.WhenAll(routerTask, systemServiceTask, routerServerTask, adsEmuTask);
    // Succeeded
