 ...
 */
public class AdsDeviceEmulator : AdsServer {
  ...
  */
  ...
  public AdsDeviceEmulator(ushort port, string portName, ILoggerFactory logger) : base(port, portName, logger) {
    _serverLogger = new ServerLogger(logger.CreateLogger<AdsDeviceEmulator>());
  }

  /*
   * Instanstiate an ADS server with an unfixed ADS port assigned by the ADS router.
   */
  public AdsDeviceEmulator(string portName, ILoggerFactory factory) : base(portName, factory) {
    _serverLogger = new ServerLogger(factory.CreateLogger<AdsDeviceEmulator>());
  ...
  protected override void OnConnected() {
    _serverLogger.Logger.LogInformation($"Server '{GetType()}', Address: {base.ServerAddress} connected!");
  }
  ...
  protected override Task<ResultWrite> OnWriteAsync(AmsAddress sender, uint invokeId, uint indexGroup, uint indexOffset, ReadOnlyMemory<byte> writeData, CancellationToken cancel) {
    ...
    /* use index group (and offset) to distinguish between the services of this server */
    switch (indexGroup) {
      case 0x10000:
  ...
  protected override Task<ResultHandle> OnAddDeviceNotificationAsync(AmsAddress sender, uint invokeId, uint indexGroup, uint indexOffset, int dataLength, AmsAddress receiver, NotificationSettings settings, CancellationToken cancel) {
    ...
    var notEntry = new NotificationRequestEntry(receiver, indexGroup, indexOffset, dataLength, settings);
